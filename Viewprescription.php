...
$selectSQL = "SELECT * FROM prescription INNER JOIN pet_owners ON pet_owners.Id = prescription.OwnerID INNER JOIN doctor ON doctor.Doc_id = prescription.Doc_id INNER JOIN pets ON pets.PetID = prescription.PetID WHERE pet_owners.email='$email' ";
...
if($nor1>0){
  while ($row1= mysqli_fetch_array($r1)){
    //print_r($row1);
    $PetID=$row1['PetID'];
    $IssuedDate=$row1['IssuedDate'];
    $app_ID=$row1['app_ID'];
    $Image=$row1['Image'];
    $Doc_id=$row1['Doc_id'];
    $OwnerID=$row1['OwnerID'];
    $PetName=$row1['PetName'];}}
            ...
            <div class="table-responsive">
                <div class="table-wrapper">
                    <div class="table-title">
                        <div class="row">
                            <div class="col-sm-5">
                                <h2>Pet <b>Prescription</b></h2>
                            </div>
                            <?php
                            $vid=$_GET['View_id'];
                            $ret=mysqli_query($conn,"select * from prescription where PetID =$vid");
                            $cnt=1;
...
<!--Fetch the Records -->
                    <tr>
                        ...
                        <td><td><img src="./prescription/<?php  echo $row['Image'];?>" width="120" height="120"></td></td>
