...
<body>
  ...
  function sendMail($email)
  {
    ...
    $subject = "";
    if ($email) {
      // come here when doctor accepted (if true )
      $msg = "doctor is request accepted";
      $subject = "Clinic Appoiment- Accepted";
    } else {
      // else rejected this email will pass(if false )
    ...
    $mail->isSMTP();
    $mail->Host = 'smtp.gmail.com';//mail service provider
    $mail->SMTPAuth = true;
    ...
    $mail->msgHTML($msg);
    $mail->addAddress(trim($_GET['email']));// petowner email get by appointment
    if (!$mail->send()) {
